...
Generate a minimal user-facing project config in the current directory:

```bash
forge init
```

This writes `forge.yml`. If the file already exists, Forge fails safely unless you pass:
...
- `forge.yml` is the user-facing config generated by `forge init`
- current deploy and webhook flows still read `forge.project.json`
